...
class Listado {
	...
	public function imprimirRanking() {
		...
		while ( $this->lista = mysql_fetch_array ( $registros ) ) {
			...
			$odd = ($odd == "odd") ? "" : "odd";
			echo "<tr class='$odd' style='height:32px;'>
					<td style='background-color:white; padding-right:10px;'><b>$posicion</b></td>
					<td><img src=\"files/profiles/$apellido.jpg\" style=\"width:30px; height:30px; cursor:pointer;\" onclick=\"abrirImagen('files/profiles/$idJug.jpg');\"/></td>
					<td>$nombre</td>
					<td>$apellido</td>
					<td>$puntos</td>
					<td width='30px'>$pj</td>
	...
	public function verHistorial($idJug1, $idJug2){
		...
		while ( $this->lista = mysql_fetch_array ( $registros ) ) {
			...
			$imgJug1 = "files/profiles/" . $apellidoJug1 . ".jpg";
			$imgJug2 = "files/profiles/" . $apellidoJug2 . ".jpg";
